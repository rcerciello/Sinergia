...
import static it.rcerciello.weekLibrary.weekview.WeekViewUtil.today;
...
public class WeekView extends View {
    ...
    private SinergiaScrollListener mScrollListener;
    ...
    public void setScrollXParameters(Direction currentScrollDirection, float distanceX, float velocityX) {
        Timber.i("setScrollXParameters");
        mCurrentScrollDirection = currentScrollDirection;
        mGestureListener.onScroll(null, null, distanceX, mdistanceY);
    }


    public void setScrollYParameters(Direction currentScrollDirection, float distanceY, float velocityY) {
        Timber.i("setScrollYParameters");
        mCurrentScrollDirection = currentScrollDirection;
        mGestureListener.onScroll(null, null, mdistanceX, distanceY);
    }
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            switch (mCurrentScrollDirection) {
                ...
                case RIGHT:
                    ...
                    mScrollListener.onMyScrollXListener(Direction.RIGHT, mCurrentOrigin.x);
                    break;
                case VERTICAL:
                    mCurrentOrigin.y -= distanceY;
                    ViewCompat.postInvalidateOnAnimation(WeekView.this);
//                    mScrollListener.onMyScrollXListener(Direction.VERTICAL,   mCurrentOrigin.y);
                    break;
            }
            return true;
        }
...
//                case LEFT:
//                case RIGHT:
//                    mScrollListener.onMyScrollXListener(Direction.RIGHT, (int) (velocityX * mXScrollingSpeed));
...
//                    break;
//                case VERTICAL:
//                    mScrollListener.onMyScrollYListener(Direction.VERTICAL, (int) (velocityY * mXScrollingSpeed));
...
//            ViewCompat.postInvalidateOnAnimation(WeekView.this);
    ...
    private void drawTimeColumnAndAxes(Canvas canvas) {
        ...
        Timber.i("drawTimeColumnAndAxes");
        Timber.i("mHeaderColumnBackgroundPaint : "+mHeaderColumnBackgroundPaint);
        Timber.i("mHeaderHeight + mHeaderRowPadding * 2 : "+(mHeaderHeight + mHeaderRowPadding * 2));
        Timber.i("mHeaderColumnWidth : "+(mHeaderColumnWidth));
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            int i = 7;
            for (int hourNumber = 7; hourNumber < 24; hourNumber++) {
    ...
    private void drawEvents(Calendar date, float startFromPixel, Canvas canvas) {
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            Timber.i("Draw Events");
            for (int i = 0; i < mEventRects.size(); i++) {
                Timber.i("Draw Events | mEventRects.size() = "+mEventRects.size()
                );
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) && !mEventRects.get(i).event.isAllDay()){
                    ...
                    if (left < right &&
                        ...
                        Timber.i("row 815");
                        Timber.i("EventRegt.get("+i+").event.name )>"+mEventRects.get(i).event.getAppointmentName());
    ...
    private void drawAllDayEvents(Calendar date, float startFromPixel, Canvas canvas) {
        Timber.i("draw all day events");
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                Timber.i("draw all day events | mEventRects.size() : "+mEventRects.size());
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) && mEventRects.get(i).event.isAllDay()){
    ...
    private void drawEventTitle(WeekViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
        ...
        if (event.getAppointmentName() != null) {
            bob.append(event.getAppointmentName());
...
//                mPreviousPeriodEvents = previousPeriodEvents;
    ...
    private void expandEventsToMaxWidth(List<EventRect> collisionGroup) {
        ...
        for (List<EventRect> column : columns){
          ...
          Timber.i("maxRowCount => "+maxRowCount);
        }
    ...
    public void setScrollListener(SinergiaScrollListener scrolledListener){
        this.mScrollListener = scrolledListener;
    }

    public SinergiaScrollListener getScrollListener(){
    ...
    public interface SinergiaScrollListener {
        /**
